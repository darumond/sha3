...
# Expected hashes for different SHA3 variants
EXPECTED_HASHES = {
    224: "e642824c3f8cf24ad09234ee7d3c766fc9a3a5168d0c94ad73b46fdf",  # Example
...
def run_sha3_tool(file_path):
    # Execute the binary with the file as input
...
def extract_hash(output, hashBitLength):
    # Search for the line containing "SHA3-<bitLength> Hash:" and capture the next line
    lines = output.splitlines()
    for i in range(len(lines)):
        if f"SHA3-{hashBitLength} Hash:" in lines[i]:
            # The hash is on the line immediately following
...
def main():
    ...
    # Check the hashes for all bit lengths
